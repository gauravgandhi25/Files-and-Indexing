...
class Record {
	...
	public String toString() {
		return "Record [" + id + ", " + company + ", " + new String(drugid) + ", " + trials + ", "
				...
				+ controlled_study + ", " + govt_funded + ", " + fda_approved + "]";
	}
...
public class MyDatabase {
	...
	private static void updateBooleanIndexFiles(File indexFile,
			TreeMap<Boolean, TreeSet<Integer>> indexMap) {
		BufferedWriter bw;
		try {
	...
	private static void updateShortIndexFiles(File indexFile,
			TreeMap<Short, TreeSet<Integer>> indexMap) {
		...
		while (true) {
			...
			case 2:
				if (dbExist())
					...
					System.out
							.println("Input File Is Not Yet Parsed. Select Option 1 before querying fields");
				break;
			case 3:
...
//		int comparison = getCompareChoice();
	...
	public static Record readSingleRecord(int baseAddress) {
		...
		try {
			...
			record = new Record(id, company, drugid, trials, patients, dosage_mg, reading,
					double_blind, controlled_study, govt_funded, fda_approved);
			record.display();
			file.close();
		} catch (FileNotFoundException e) {
			e.printStackTrace();
	...
	private static void loadBooleanIndexMaps(File indexFile,
			TreeMap<Boolean, TreeSet<Integer>> indexMap) throws IOException {
